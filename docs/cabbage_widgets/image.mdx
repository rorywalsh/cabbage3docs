import Font from "./properties/font.mdx";
import Channel from "./properties/channel.mdx";
import Colour from "./properties/colour.mdx";
import Opacity from "./properties/opacity.mdx";
import Bounds from "./properties/bounds.mdx";
import Visible from "./properties/visible.mdx";
import Active from "./properties/active.mdx";
import Automatable from "./properties/automatable.mdx";
import Sample from "./properties/sample.mdx";
import File from "./properties/fileImage.mdx";
import TableNumber from "./properties/tableNumber.mdx";
import Text from "./properties/text.mdx";
import Id from "./properties/id.mdx";
import CodeBlock from '@theme/CodeBlock';
import widgetCode from '!!raw-loader!../../static/examples/image.csd';
import StyleImage from "./image.style.mdx";

# Image

`image` creates a simple on-screen text shape. Users can interact with an image via mouse down events if a channel name has been assigned. 

# List of supported properties
[Active](#active) | [Automatable](#automatable) | [Bounds](#bounds) | [Channel](#channel) | [Colour](#colour) | [Font](#font) |[Id](#id) | [File](#file) | [Opacity](#opacity) |[Visible](#visible) | [Style](#style)


### Unique Properties

<a id="file" class="anchor-target"></a>
#### File
<File />

### Common Properties

<a id="active" class="anchor-target"></a>
#### Active
<Active />

<a id="bounds" class="anchor-target"></a>
#### Bounds
<Bounds />

<a id="channel" class="anchor-target"></a>
#### Channel
<Channel />

<a id="colour" class="anchor-target"></a>
#### Colour
<Colour />

<a id="font" class="anchor-target"></a>
#### Font
<Font />

<a id="id" class="anchor-target"></a>
#### Id
<Id />

<a id="opacity" class="anchor-target"></a>
#### Opacity
<Opacity />

<a id="text" class="anchor-target"></a>
#### Text
<Text />

<a id="visible" class="anchor-target"></a>
#### Visible
<Visible />

<a id="automatable" class="anchor-target"></a>
#### Automatable
<Automatable />

<a id="style" class="anchor-target"></a>
#### Style
<StyleImage />


## Example

<CodeBlock language="csharp">{widgetCode}</CodeBlock>